...
const queryGetProduct = `SELECT product_id, name, description, product_slug, price, image_url FROM product WHERE product_slug = $1`

const queryDeleteProduct = `UPDATE product SET hidden = true WHERE product_slug = $1`

const queryUpdateCanteenBalance = `INSERT INTO canteen_balance (uid, amount, type) VALUES ($1, $2, $3)`
...
func (r *KantinDB) UpdateCanteenBalance(in entities.UpdateCanteenBalanceRequest) (entities.UpdateCanteenBalanceResponse, error) {
	...
	_, err := r.Db.Exec(queryUpdateCanteenBalance, in.UID, in.Amount, in.Type)
